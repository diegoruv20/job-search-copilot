...
def normalize_sankey_data(data):
    ...
    legacy_nodes = {
        ...
        "Rejected after technical stage",
    ...
    if not legacy_nodes.intersection(node_names):
        return data
    ...
    for link in data.get("links", []):
        ...
        legacy_links[(source, target)] = legacy_links.get((source, target), 0) + link[
            "value"
        ]
    ...
    applied_count = legacy_links.get(("Tracked roles", "Applied"), 0)
    add("Tracked roles", "Applied", applied_count)
    add("Applied", "Resume review", applied_count)

    assigned_count = 0
    awaiting_count = legacy_links.get(("Applied", "Active / awaiting response"), 0)
    add("Resume review", "No response yet", awaiting_count)
    assigned_count += awaiting_count

    withdrawn_count = legacy_links.get(("Applied", "Withdrawn"), 0)
    add("Resume review", "Withdrawn", withdrawn_count)
    assigned_count += withdrawn_count

    offer_count = legacy_links.get(("Applied", "Offer"), 0)
    add("Resume review", "Advanced to interviews", offer_count)
    add("Advanced to interviews", "Final interview", offer_count)
    add("Final interview", "Offer", offer_count)
    assigned_count += offer_count

    active_interview_count = sum(
        legacy_links.get(("Applied", stage), 0)
        for stage in (
            "Recruiter screen",
            "Hiring manager",
            "Technical interview",
    ...
    add("Resume review", "Advanced to interviews", active_interview_count)
    add("Advanced to interviews", "Active interviewing", active_interview_count)
    assigned_count += active_interview_count

    final_active_count = legacy_links.get(("Applied", "Onsite"), 0)
    add("Resume review", "Advanced to interviews", final_active_count)
    add("Advanced to interviews", "Final interview", final_active_count)
    add("Final interview", "Final interview active", final_active_count)
    assigned_count += final_active_count

    rejected_count = legacy_links.get(("Applied", "Rejected"), 0)
    resume_rejections = legacy_links.get(
        ("Rejected", "Rejected at resume screen"), 0
    )
    interview_rejections = sum(
        legacy_links.get(("Rejected", reason), 0)
        for reason in (
            "Rejected after recruiter screen",
            "Rejected after technical stage",
        )
    )
    final_rejections = legacy_links.get(("Rejected", "Rejected after onsite"), 0)
    detailed_rejections = (
        resume_rejections + interview_rejections + final_rejections
    )
    resume_rejections += max(0, rejected_count - detailed_rejections)

    add("Resume review", "Rejected at resume review", resume_rejections)
    add("Resume review", "Advanced to interviews", interview_rejections)
    add(
        "Advanced to interviews",
        "Rejected during interviews",
        interview_rejections,
    )
    add("Resume review", "Advanced to interviews", final_rejections)
    add("Advanced to interviews", "Final interview", final_rejections)
    add(
        "Final interview",
        "Rejected after final interview",
        final_rejections,
    )
    assigned_count += rejected_count

    add(
        "Resume review",
        "No response yet",
        max(0, applied_count - assigned_count),
    )

    for (source, target), value in legacy_links.items():
        if (source, target) == ("Tracked roles", "Not applied"):
            add(source, target, value)
        elif source in {"Not applied", "Not a fit"}:
            add(source, target, value)
    ...
    return {
        "nodes": [{"name": name} for name in normalized_names],
        "links": [
            {
                "source": normalized_names.index(source),
                "target": normalized_names.index(target),
                "value": value,
            }
            for (source, target), value in links.items()
        ],
...
def sankey_snapshot_payload(snapshot):
    payload = snapshot.to_dict()
    payload.update(normalize_sankey_data(snapshot.data))
    return payload
